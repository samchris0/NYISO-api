...
from nyiso_api.schemas.day_ahead_lbmp_zonal import DayAheadLBMPZonalValidation
from nyiso_api.models.day_ahead_lbmp_zonal import DayAheadLBMPZonalModel

logger = logging.getLogger(__name__)

def scrape_day_ahead_lbmp_zonal(daterange):
    
    """
    This function gets day ahead lbmp zonal data from the NYISO archive and store requested data
    in the day ahead lbmp zonal database
    ...
    for date in year_months_days:
        valid_filenames.append(str(date[0]) + str(date[1]) + str(date[2]) + "damlbmp_zone.csv")
    ...
    for date in year_months:
        urls.append('https://mis.nyiso.com/public/csv/damlbmp/' + str(date[0]) + str(date[1]) + '01damlbmp_zone_csv.zip')
    ...
    try:
        validated_records = DayAheadLBMPZonalValidation().load(records, many=True)
    except ValidationError:
        logger.exception("Day-ahead zonal LBMP validation failed")
        raise

    if not validated_records:
        logger.info("No day-ahead zonal LBMP records found for %s", sorted(daterange))
        return
    ...
    table = DayAheadLBMPZonalModel.__table__
    chunk_size = 5_000
    try:
