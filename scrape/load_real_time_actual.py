...
def scrape_load_real_time_actual(daterange):
    ...
    try:
        ...
        logger.exception("Real-time actual load validation failed")
        raise

    if not validated_records:
        logger.info("No real-time actual load records found for %s", sorted(daterange))
        return
    ...
    table = LoadRealTimeActualModel.__table__
    chunk_size = 5_000
    try:
