...
from nyiso_api.schemas.day_ahead_lbmp_generator import DayAheadLBMPGeneratorValidation
...
def scrape_day_ahead_lbmp_generator(daterange):
    ...
    try:
        ...
        logger.exception("Day-ahead generator LBMP validation failed")
        raise

    if not validated_records:
        logger.info("No day-ahead generator LBMP records found for %s", sorted(daterange))
        return
    ...
    table = DayAheadLBMPGeneratorModel.__table__
    chunk_size = 5_000
    try:
