...
def scrape_day_ahead_ancillary(daterange):
    ...
    try:
        ...
        logger.exception("Day-ahead ancillary validation failed")
        raise

    if not validated_records:
        logger.info("No day-ahead ancillary records found for %s", sorted(daterange))
        return
    ...
    table = DayAheadAncillaryModel.__table__
    chunk_size = 5_000
    try:
