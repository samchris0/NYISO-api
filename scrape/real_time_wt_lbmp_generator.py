...
def scrape_real_time_wt_lbmp_generator(daterange):
    """
    This function gets real time wieghted lbmp generator data from the NYISO archive and store requested data
    in the real time weighted lbmp generator database
    ...
    if len(daterange) == 1 and daterange == {today}:
        ...
        for date in year_months_days:
            valid_filenames.append(str(date[0]) + str(date[1]) + str(date[2]) + "rtlbmp_gen.csv")
        ...
        for date in year_months:
            urls.append('https://mis.nyiso.com/public/csv/rtlbmp/' + str(date[0]) + str(date[1]) + '01rtlbmp_gen_csv.zip')
    ...
    try:
        validated_records = RealTimeWT_LBMPGeneratorValidation().load(records, many=True)
    except ValidationError:
        logger.exception("Real-time weighted generator LBMP validation failed")
        raise

    if not validated_records:
        logger.info("No real-time weighted generator LBMP records found for %s", sorted(daterange))
        return
    ...
    table = RealTimeWT_LBMPGeneratorModel.__table__
    chunk_size = 5_000
    try:
