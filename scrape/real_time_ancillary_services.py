...
def scrape_real_time_ancillary(daterange):
    """
    ...
    for date in year_months_days:
        valid_filenames.append(str(date[0]) + str(date[1]) + str(date[2]) + "rtasp.csv")
    ...
    for date in year_months:
        urls.append('https://mis.nyiso.com/public/csv/rtasp/' + str(date[0]) + str(date[1]) + '01rtasp_csv.zip')
