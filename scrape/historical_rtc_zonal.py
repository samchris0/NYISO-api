...
def scrape_historical_rtc_zonal(daterange):
    
    """
    This function gets historical rtc zonal data from the NYISO archive and store requested data
    in the historical rtc zonal database
    ...
    for date in year_months_days:
        valid_filenames.append(str(date[0]) + str(date[1]) + str(date[2]) + "hamlbmp_zone.csv")
    ...
    for date in year_months:
        urls.append('https://mis.nyiso.com/public/csv/hamlbmp/' + str(date[0]) + str(date[1]) + '01hamlbmp_zone_csv.zip')
    ...
    try:
        validated_records = HistoricalRTCZonalValidation().load(records, many=True)
    except ValidationError:
        logger.exception("Historical RTC zonal validation failed")
        raise

    if not validated_records:
        logger.info("No historical RTC zonal records found for %s", sorted(daterange))
        return
    ...
    table = HistoricalRTCZonalModel.__table__
    chunk_size = 5_000
    try:
