...
class LoadBidZonalQuery(Schema):
    ...
    def start_after(self, data, **kwargs):
        if data['start'] < datetime.datetime(2001, 6, 28, 0, 0):
...
class LoadBidZonalValidation(Schema):
    timestamp = fields.DateTime(required=True)
    name = fields.String()
    ptid = fields.Integer()
    energy_bid_load = fields.Float()
    bilateral_load = fields.Float()
    price_cap_load = fields.Float()
    virtual_load = fields.Float()
    virtual_supply = fields.Float()
